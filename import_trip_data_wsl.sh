...
for filename in data/green_tripdata*.csv; do
  ...
  sed 's/\r//' $filename | sed '/^$/d' | psql.exe -c "COPY green_tripdata_staging ${schema} FROM stdin CSV HEADER;" nyc-taxi-data
  echo "`date`: finished raw load for ${filename}"
  psql.exe -f setup_files/populate_green_trips.sql nyc-taxi-data
...
for filename in data/yellow_tripdata*.csv; do
  ...
  echo "`date`: beginning load for ${filename}"
  sed 's/\r//' $filename | sed '/^$/d' | psql.exe -c "COPY yellow_tripdata_staging ${schema} FROM stdin CSV HEADER;" nyc-taxi-data
  echo "`date`: finished raw load for ${filename}"
  psql.exe -f setup_files/populate_yellow_trips.sql nyc-taxi-data
  echo "`date`: loaded trips for ${filename}"
done;

psql.exe -c "CREATE INDEX ON trips USING BRIN (pickup_datetime) WITH (pages_per_range = 32);" nyc-taxi-data
