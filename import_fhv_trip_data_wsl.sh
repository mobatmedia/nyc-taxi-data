...
for filename in data/fhv_tripdata*.csv; do
  ...
  cat $filename | psql.exe -c "COPY fhv_trips_staging ${fhv_schema} FROM stdin CSV HEADER;" nyc-taxi-data
  echo "`date`: finished raw load for ${filename}"
  psql.exe -f setup_files/populate_fhv_trips.sql nyc-taxi-data
...
for filename in data/fhvhv_tripdata*.csv; do
  echo "`date`: beginning load for ${filename}"
  cat $filename | psql.exe -c "COPY fhv_trips_staging ${fhvhv_schema} FROM stdin CSV HEADER;" nyc-taxi-data
  echo "`date`: finished raw load for ${filename}"
  psql.exe -f setup_files/populate_fhv_trips.sql nyc-taxi-data
  echo "`date`: loaded trips for ${filename}"
done;

psql.exe -c "CREATE INDEX ON fhv_trips USING BRIN (pickup_datetime) WITH (pages_per_range = 32);" nyc-taxi-data
